...
void Plantower_PMS7003::updateFrame() {
  if (!initialized) {
    USB.println("Error: must call Plantower_PMS7003::init()");
    return;
  }
  dataReady = false;

  if (serialAvailable(1)) {
    nextByte = serialRead(1);
  ...
  if (bufferIndex == PMS7003_DATA_SIZE) {
    if (sensorData.bytes[0] == 0x42 && sensorData.bytes[1] == 0x4d) {
      ...
      if(isValidChecksum()) {
        ...
        if (debug) {
          USB.println("Invalid data checksum");
        }
      }
    } else {
      if (debug) {
        USB.println("Malformed first byte");
      }
    }
    bufferIndex=0;
  }   
